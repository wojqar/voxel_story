...
fn is_transparent(
    ...
    let neighbor = if x < 0       { neighbors.nx }
                   else if x >= s { neighbors.px }
                   else if y < 0  { neighbors.ny }
                   else if y >= s { neighbors.py }
                   else if z < 0  { neighbors.nz }
                   else           { neighbors.pz };
    match neighbor {
        None    => true,
        Some(n) => n.get(
            ...
            for u in 0..CHUNK_SIZE {
                for v in 0..CHUNK_SIZE {
                    ...
                    'outer: while u + height < CHUNK_SIZE {
                        for k in 0..width {
                            if visited[u + height][v + k]
                                || mask[u + height][v + k] != voxel
                            {
                                break 'outer;
                            }
                        }
...
fn spawn_mesh_tasks(
    ...
    for (entity, chunk_entity) in query.iter() {
        ...
        let Some(chunk) = world.get_chunk(coord) else { continue };
...
fn apply_mesh_tasks(
    ...
    for (entity, chunk_entity, mut task) in query.iter_mut() {
        let Some(mesh) = future::block_on(future::poll_once(&mut task.0)) else { continue };
